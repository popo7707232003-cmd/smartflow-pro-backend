...
import { Pool } from 'pg';
import { getTrackerStatus } from './signalTracker';
let pool: Pool;
const router = Router();
...
router.get('/debug/db-check', async (_req, res) => {
  ...
  try {
    ...
    if(r.sr_exists){
      r.sr_cols = (await pool.query("SELECT column_name, data_type, is_nullable FROM information_schema.columns WHERE table_name='signal_results' ORDER BY ordinal_position")).rows;
      r.sr_count=(await pool.query("SELECT COUNT(*)::int as c FROM signal_results")).rows[0].c;
      r.sr_dist=(await pool.query("SELECT result,COUNT(*)::int as c FROM signal_results GROUP BY result")).rows;
      r.sr_latest=(await pool.query("SELECT * FROM signal_results ORDER BY closed_at DESC LIMIT 1")).rows[0];
    }
